...
const ChatRoomFooter = () => {
    ...
    const receiverId = chatRoomData.userId;
    ...
    const sendMessage = async () => {
        ...
        console.log("채팅방이 없을 경우 senderId: ", receiverId);
       // TODO: 웹소켓 구독때문에, 채팅방 id가 없는 경우와 있는 경우로 나눌수 밖에 없음. -> 백엔드에서 한번에 처리 불가.
        if(!chatRoomId) {
            ...
            const response = await createDirectChatRoom(receiverId);
            const chatRoomPayload: ChatRoomPayload = {
                ...
                friendId: receiverId,  // 응답에서 받은 friendId
            ...
            sendChatMessageViaSocket(response.id, message, receiverId);
        } else {
            // 기존 채팅방 있는 경우 바로 실시간 통신
            sendChatMessageViaSocket(chatRoomId ?? 0, message, receiverId);
        }
        setMessage("");
    }
